	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
                ...
                //o.noiseColor = fixed4(c,c,c,1);
                float v = _Volume * 3 -  1.5; // 设定值在[-1.5,1.5] 区间内
                //v = v * 3 - 1.5;//(((_Volume) - 0) * (1 - 0)) * (1.5 - -1.5) + -1.5;
                //v = (((_Volume) - 0) * (1 - 0)) * (1.5 - -1.5) + -1.5;
                //float v = (((_Volume) - 0) * (1 - 0)) * (1.5 - -1.5) + -1.5;
                clip(c - v);
                ...
                //clip(i.noiseColor.r - _Volume);
                //float ccc = (c + 1.5) /3;
			    //col = fixed4(ccc ,ccc,ccc,1); //i.noiseColor;
				//return col;
                //return _Color;
                return col;
			}
